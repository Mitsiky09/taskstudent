...
void main() {
  ...
  group('saisie rapide', () {
    test('une phrase devient une tâche structurée', () async {
      final container = makeContainer(MemoryStore());
      addTearDown(container.dispose);
      final notifier = container.read(tasksProvider.notifier);
      await notifier.load();

      final created = await notifier.createFromQuickAdd(
        'Rendre le devoir demain à 14h30 #Travail p1',
        categories: kProjects,
      );

      expect(created, isNotNull);
      expect(created!.title, 'Rendre le devoir');
      expect(created.priority, Priority.high);
      expect(created.subjectId, 'work');
      expect(created.due.hour, 14);
      expect(created.due.minute, 30);
    });

    test('la récurrence reconnue est reportée sur la tâche', () async {
      final container = makeContainer(MemoryStore());
      addTearDown(container.dispose);
      final notifier = container.read(tasksProvider.notifier);
      await notifier.load();
      ...
      final created = await notifier.createFromQuickAdd(
        'Cours tous les lundis à 9h',
        categories: kProjects,
      );

      expect(created?.repeat, RepeatRule.weekly);
    });

    test('sans titre, rien n’est créé', () async {
      final container = makeContainer(MemoryStore());
      addTearDown(container.dispose);
      final notifier = container.read(tasksProvider.notifier);
      await notifier.load();

      expect(await notifier.createFromQuickAdd('   demain  ', categories: kProjects), isNull);
      expect(container.read(tasksProvider).tasks, isEmpty);
    });

    test('repeatFromRrule ramène un intervalle au cycle correspondant', () {
      expect(repeatFromRrule('FREQ=DAILY;INTERVAL=2'), RepeatRule.daily);
      expect(repeatFromRrule('FREQ=MONTHLY;BYMONTHDAY=10'), RepeatRule.monthly);
      expect(repeatFromRrule(null), RepeatRule.none);
    });
  });
