...
/// Traduit une règle RRULE en [RepeatRule].
///
/// Le noyau ne connaît que trois cycles : un intervalle (`INTERVAL=2`) est
/// ramené au cycle correspondant, sans multiplicateur.
RepeatRule repeatFromRrule(String? rrule) {
  if (rrule == null) return RepeatRule.none;
  if (rrule.startsWith('FREQ=DAILY')) return RepeatRule.daily;
  if (rrule.startsWith('FREQ=WEEKLY')) return RepeatRule.weekly;
  if (rrule.startsWith('FREQ=MONTHLY')) return RepeatRule.monthly;
...
class TasksNotifier extends Notifier<TasksState> {
  ...
  /// Crée une tâche depuis une phrase de saisie rapide.
  ///
  /// L'analyse est faite par le noyau ([parseQuickAdd]) : ce ViewModel ne
  /// connaît aucune règle de parsing, il traduit le résultat en [TaskInput].
  /// Renvoie `null` si la phrase ne contient aucun titre.
  ///
  /// Deux écarts assumés avec `components/QuickAdd.tsx` :
  ///
  /// * la récurrence reconnue (« tous les lundis ») est reportée sur
  ///   [Task.repeat], alors que l'application d'origine l'affiche dans une
  ///   pastille puis la perd à la création ;
  /// * aucun rappel n'est programmé (pas de plugin de notifications ici).
  Future<Task?> createFromQuickAdd(String input, {required List<Subject> categories}) async {
    ...
    final due = parsed.due;
    return create(
      TaskInput(
        title: parsed.title,
        description: parsed.description,
        dueDate: due == null ? endOfDay(DateTime.now()) : dueToDate(due),
        priority: parsed.priority,
        subjectId: parsed.subject?.id ?? kFallbackProject.id,
        repeat: repeatFromRrule(due?.rrule),
      ),
    );
  }
